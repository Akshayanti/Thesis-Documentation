\chapter{Mining Errors in Low-Resource Languages by Combining LISCA And Cross-Validation (Experiment 3)}
\label{chap:lisca}
...
\begin{reusefigure}[H]{fig:lisca_stats}
    \centering
    \includegraphics[scale=0.4]{img/lisca_stats.png}
...
\label{algo:liscadata}
    ...
    \FORALL{$k$ in \{2, 4, 8\}}
        ...
        \STATE \hspace{5mm} $T = \bigcup \{T.1, ..., T.k\}$ \COMMENT{Condition 1}
        \STATE \hspace{5mm} $sentences(T.i) = sentences(T)/k \hspace{2mm} \forall T.i \in T$
        \COMMENT{Condition 2}
        \STATE \hspace{5mm} $\bigcap \{T.x1, T.x2\} = \phi \hspace{2mm} \forall \{T.x1, T.x2\}  \in T.folds$ \COMMENT{Condition 3}
